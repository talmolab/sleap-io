...
class TestSkeletonDecoder:
    ...
    def test_decode_real_skeleton_file(self):
        ...
        skeleton = sio.load_skeleton("tests/data/slp/labels.v002.rel_paths.skeleton.json")
...
class TestRoundTrip:
    ...
    def test_real_file_round_trip(self, tmp_path):
        ...
        # Load original
        skeleton1 = sio.load_skeleton("tests/data/slp/labels.v002.rel_paths.skeleton.json")
        ...
        assert skeleton1.name == skeleton2.name
        assert len(skeleton1.nodes) == len(skeleton2.nodes)
        assert all(n1.name == n2.name for n1, n2 in zip(skeleton1.nodes, skeleton2.nodes))
        assert len(skeleton1.edges) == len(skeleton2.edges)