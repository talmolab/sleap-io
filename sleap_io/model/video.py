...
class Video:
    ...
    def is_same_file(self, other: "Video") -> bool:
        """Check if this video refers to the same file as another video.

        This method provides definitive file identity checking, which is different
        from matches_path() or matches_content():

        - Stricter than matches_path(strict=False): Only matches if files are
          verifiably the same, not just same basename
        - Considers source_video: For embedded videos (PKG.SLP), traverses the
          source_video chain to find the original file
        - Conservative: Returns False when file identity cannot be verified
          (e.g., cross-platform paths without file access)
        ...
        Notes:
            This method is intended for duplicate prevention when adding videos.
            For matching during merges (where basename matching may be acceptable),
            use matches_path() with the appropriate strictness level.
        """
        # Get the effective (root) video for each, traversing source_video chains
        self_root = self._get_root_video()
        other_root = other._get_root_video()

        # Compare the root videos directly
        return self_root._is_same_file_direct(other_root)

    def _get_root_video(self) -> "Video":
        """Traverse source_video/original_video chain to find the root video.

        Returns the root of the provenance chain:
    ...
    def _is_same_file_direct(self, other: "Video") -> bool:
        """Check if two videos (without source_video indirection) are the same file."""
        ...
        self_path = Path(self.filename)
        other_path = Path(other.filename)
        ...
        try:
            if self_path.exists() and other_path.exists():
                ...
                return os.path.samefile(self_path, other_path)
        ...
        try:
            if self_path.resolve() == other_path.resolve():
                return True
        except (OSError, ValueError):
        ...
        return str(self_path) == str(other_path)
    ...
    def shapes_compatible(self, other: "Video") -> bool | None:
        """Check if this video's shape is compatible with another video.
        ...
        """
        # Get shapes, preferring backend_metadata for videos without loaded backend
        self_shape = self._get_effective_shape()
        other_shape = other._get_effective_shape()
        ...
        return (
            self_shape[0] == other_shape[0]  # frames
            and self_shape[1] == other_shape[1]  # height
            and self_shape[2] == other_shape[2]  # width
        )

    def _get_effective_shape(self) -> tuple | None:
        ...
        if self.original_video is not None:
            original_shape = self.original_video._get_effective_shape()
    ...
    def original_videos_conflict(self, other: "Video") -> bool:
        """Check if both videos have conflicting original_video references.

        This is used for REJECTION in video matching. If both videos have
        original_video set but pointing to different files, they definitely
        ...
        Returns:
            True if both have original_video AND they point to different files.
            False otherwise (including if either or both have no original_video).

        Notes:
            This implements the "NOT MATCH" case of the original_video OR logic.
            The "MATCH" case is handled by is_same_file() which traverses chains.
        """
        self_root = self._get_root_video()
        other_root = other._get_root_video()

        # Only conflict if BOTH have non-trivial chains (original_video set)
        # AND the roots are different
        self_has_original = (
            self.original_video is not None or self.source_video is not None
        )
        other_has_original = (
            other.original_video is not None or other.source_video is not None
        )

        if not (self_has_original and other_has_original):
            # At least one has no original_video - no conflict
            return False

        # Both have original_video - check if roots are the same file
        return not self_root._is_same_file_direct(other_root)
