...
def _create_dataframe_from_rows(
    ...
    if backend == "polars":
        ...
        # sparse columns (e.g. {node}.score, present only on predicted instances)
        # are not dropped or mis-typed as Null when absent from the first rows.
        # Without this, polars' bounded inference silently drops or fails on a
